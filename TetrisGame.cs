...
namespace MyPersonalTetris
{
    public class TetrisGame : ITetrisGame
    {
        private readonly List<Tetromino> TetrisFigures = new List<Tetromino>()
            {
                new Tetromino(new bool[,] { { true, true, true, true } }),
        ...
        {
            this.CurrentFigure = this.TetrisFigures[this.random.Next(0, this.TetrisFigures.Count)];
            this.CurrentFigureRow = 0;
        ...
        {
            if (this.CurrentFigureCol > TetrisColumns - figure.Height)
            {
                return true;
            }

            if (this.CurrentFigureRow + figure.Width == TetrisRows)
            {
                return true;
            }
            ...
            for (int row = 0; row < figure.Width; row++)
            {
                ...
                {
                    if (figure.Body[row, col] && this.TetrisField[this.CurrentFigureRow + row + 1, this.CurrentFigureCol + col])
                    {
                        return true;
                    }
                }
